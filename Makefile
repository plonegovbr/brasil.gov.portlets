...
htmlcov/index.html: README.rst src/brasil.gov.portlets/*.py bin/coverage
	@bin/coverage run --source=./src/brasil.gov.portlets/ --branch bin/test
	@bin/coverage html -i
	@touch $@
...
docs/html/index.html: docs/*.rst src/brasil.gov.portlets/*.py src/brasil.gov.portlets/browser/*.py src/brasil.gov.portlets/tests/*.py bin/sphinx-build
...
clean:
	@rm -rf .coverage .installed.cfg .mr.developer.cfg bin docs/html htmlcov parts develop-eggs \
		src/brasil.gov.portlets.egg-info lib include .Python
